...
export class CompanyFormManager {
  ...
  public setValues(model: CompanyResponseModel): void {
    ...
    if (model.companyAddress) {
      this.formInstance.get('addressLine1')?.setValue(model.companyAddress.addressLine1);
      this.formInstance.get('addressLine2')?.setValue(model.companyAddress.addressLine2);
      this.formInstance.get('street')?.setValue(model.companyAddress.addressStreet);
      this.formInstance.get('country')?.setValue(model.companyAddress.addressCountry);
      this.formInstance.get('postcode')?.setValue(model.companyAddress.addressPostCode);
    }
  }
